...
ydlidar_ros2_driver depends on YDLidar-SDK. On this robot, the adjacent
`/home/ros/YDLidar-SDK` source tree is built automatically inside the ROS
package build directory. Other layouts can either set
`YDLIDAR_SDK_LOCAL_ROOT` to an SDK source tree or install the SDK system-wide.
...
### X2/X2L profile used by this robot

The default `params/ydlidar.yaml` is configured for the robot's X2/X2L:
115200 baud, triangle protocol, serial transport, 3 kHz requested sample rate,
single-channel mode, no intensity, and DTR motor control. Keep `frame_id` as
`laser_frame`; the robot URDF owns the sensor transform.

Verify the standalone driver before starting SLAM:

```
source /opt/ros/jazzy/setup.bash
source /home/ros/ydlidar_ros2_ws/install/setup.bash
ros2 launch ydlidar_ros2_driver ydlidar_launch.py
```
...
source /opt/ros/jazzy/setup.bash
source /home/ros/ydlidar_ros2_ws/install/setup.bash
ros2 topic hz /scan --window 50 --wall-time
```

Standalone launches do not respawn by default, so the first startup failure is
kept visible. Setting `respawn:=true` enables at most three retries. Static TF
publication is also opt-in because `robot_state_publisher` owns it in the full
robot stack. For a standalone parameter file whose `frame_id` is not
`laser_frame`, also pass a matching
`static_tf_child_frame:=<your_sensor_frame>` when enabling the transform.

For single-channel X2/X2L units, the SDK's baseplate-information warning is not
fatal. A fatal message saying that no valid scan revolution arrived means that
the serial port opened but the sensor supplied no usable scan data. Check that
the head spins continuously, measure the 5 V supply under load, and inspect the
LiDAR TX-to-USB-adapter RX connection.
...
| inverted     | bool | Inverted LiDAR.<br/>false -- ClockWise.<br/>true -- CounterClockWise  <br/>default: `false` |
...
| auto_intensity     | bool | Automatically detect intensity encoding.<br/>Keep `false` for X2/X2L.<br/>default: `false` |
...
| range_min     | float | Minimum Valid range.<br/>default: `0.12` |
| range_max     | float | Maximum Valid range.<br/>default: `8.0` |
| frequency     | float | Set Scanning Frequency.<br/>default: `7.0` |
